...
import type { VerifySetupMfaEmailDTO } from "~/modules/settings/application/dtos/settings.dto";
...
export function useVerifyMfaEmailMutation() {
  ...
  return useMutation({
    mutationFn: (dto: VerifySetupMfaEmailDTO) => getSettingsUseCases().verifyMfaEmail.execute(dto),
    onSuccess: () => {
